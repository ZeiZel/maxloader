...
describe('DOM integration', () => {
  it('finds selected file buttons and action panel without hashed classes', () => {
    expect(selectedItems(document)).toHaveLength(3);
    expect(findActionPanel(document)?.element.className).toBe('actions');
  ...
  it('creates one own button and updates/removes it safely', () => {
    const panel = findActionPanel(document)!.element;
    const button = createOwnButton(3, document);
    panel.append(button);
  ...
  it('excludes disabled download buttons from the count', () => {
    ...
    expect(selectedItems(document)).toHaveLength(2);
  });
});

describe('idempotent rendering', () => {
  it('carries MAX button classes', () => {
    expect(createOwnButton(2, document).className).toBe('button button--xsmall button--ghost max-loader-button');
  });
  it('does not touch the DOM when the label is unchanged', () => {
    const button = createOwnButton(2, document);
  ...
  it('restores classes stripped by the app', () => {
    const button = createOwnButton(1, document);
